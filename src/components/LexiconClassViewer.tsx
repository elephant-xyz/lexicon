...
import { LexiconClass, LexiconTag, LexiconProperty, LexiconRelationship } from '../types/lexicon';
...
const LexiconClassViewer: React.FC<LexiconClassViewerProps> = ({
  ...
  const [expandedDeprecatedProperties, setExpandedDeprecatedProperties] = useState<Set<string>>(
    new Set()
  );
  ...
  const nonDeprecatedClasses = classes.filter((cls, _index) => !cls.is_deprecated);
  const deprecatedClasses = classes.filter((cls, _index) => cls.is_deprecated);
        ...
        <div className="method-list-item-content">
          ...
          {(() => {
            ...
            const hasVisibleProperties = Object.entries(cls.properties || {}).some(([propName]) =>
              shouldShowProperty(cls, propName)
            );
            ...
            const nonDeprecatedProps = Object.entries(cls.properties || {}).filter(
              ([propName]) =>
                shouldShowProperty(cls, propName) && cls.deprecated_properties?.[propName] !== true
            );
                            ...
                            {[...propData.enum]
                              ...
                              .map((value, idx) => {
                                ...
                                const isDeprecatedEnum =
                                  Array.isArray(deprecatedValue) && deprecatedValue.includes(value);
                        ...
                        return (
                          <div className="deprecated-values-section">
                            <button
                              ...
                              aria-label={
                                isExpanded
                                  ? 'Collapse deprecated values'
                                  : 'Expand deprecated values'
                              }
                            >
                              <span className="deprecated-values-label">Deprecated Values:</span>
                              <span className="deprecated-values-arrow">
                                {isExpanded ? '−' : '+'}
                              </span>
                            </button>
                        ...
                        <button
                          ...
                          aria-label={
                            expandedDeprecatedProperties.has(cls.type)
                              ? 'Collapse deprecated properties'
                              : 'Expand deprecated properties'
                          }
                        >
                          <span className="deprecated-properties-arrow">
                            {expandedDeprecatedProperties.has(cls.type) ? '−' : '+'}
                          </span>
                        </button>
                      </div>
            ...
            const renderRelationship = (
              ...
              return (
                ...
                >
                  <div className="method-list-item-label">
                    <div className="method-list-item-label-name">
                      <span className="relationship-name">{relName}</span>
                    </div>
                    <div className="method-list-item-label-description">
                      {relData.comment || ''}
                    </div>
          ...
          <button
            ...
            aria-label={
              expandedDeprecatedClasses
                ? 'Collapse deprecated classes'
                : 'Expand deprecated classes'
            }
          >
            <span className="deprecated-classes-label">Deprecated Classes:</span>
            <span className="deprecated-classes-arrow">
              {expandedDeprecatedClasses ? '−' : '+'}
            </span>
          </button>
