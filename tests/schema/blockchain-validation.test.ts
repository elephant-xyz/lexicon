  ...
  const checkPropertyConstraints = (property: any, propertyName: string): string[] => {
    ...
    if (property.properties) {
      for (const [nestedPropName, nestedProp] of Object.entries(property.properties)) {
        const nestedIssues = checkPropertyConstraints(nestedProp, `${propertyName}.${nestedPropName}`);
    ...
    for (const className of BLOCKCHAIN_CLASSES) {
      ...
      try {
        ...
        for (const [propertyName, property] of Object.entries(schema.properties)) {
          ...
          if (typedProperty.type === 'string' || (Array.isArray(typedProperty.type) && typedProperty.type.includes('string'))) {
            if (typedProperty.minLength !== 1) {
              allIssues.push(`${className}.${propertyName}: Expected minLength: 1, got: ${typedProperty.minLength}`);
            }
          }
        }
      } catch (error) {
    ...
    for (const className of BLOCKCHAIN_CLASSES) {
      ...
      try {
        ...
        for (const [propertyName, property] of Object.entries(schema.properties)) {
          ...
          if (typedProperty.type === 'integer' || (Array.isArray(typedProperty.type) && typedProperty.type.includes('integer'))) {
            if (typedProperty.minimum !== 1) {
              allIssues.push(`${className}.${propertyName}: Expected minimum: 1, got: ${typedProperty.minimum}`);
            }
          }
        }
      } catch (error) {
        ...
        for (const [propertyName, property] of Object.entries(schema.properties)) {
          ...
          if (typedProperty.type === 'array') {
            if (typedProperty.minItems !== 1) {
              allIssues.push(`${className}.${propertyName}: Expected minItems: 1, got: ${typedProperty.minItems}`);
            }
          }
        }
      } catch (error) {
          ...
          if (Array.isArray(typedProperty.type) && typedProperty.type.length > 1) {
            ...
            for (const nonNullType of nonNullTypes) {
              if (nonNullType === 'string' && typedProperty.minLength !== 1) {
                allIssues.push(`${className}.${propertyName}: Nullable string should have minLength: 1`);
              }
              if (nonNullType === 'integer' && typedProperty.minimum !== 1) {
                allIssues.push(`${className}.${propertyName}: Nullable integer should have minimum: 1`);
              }
            }
          }
        }
